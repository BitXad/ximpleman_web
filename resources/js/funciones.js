...
// Convierte valores JS a literales SQL seguros para las cadenas que este módulo envía en datos1/cad.
// Evita que productos como MAC'S o textos con comillas rompan el INSERT armado en PHP.
...
function inicio(){
        ...
        let tiposistema = document.getElementById('parametro_tiposistema').value; //tipo sistema SFV =1; > 1 eletronico y computarizado 
        let parametro_tipoemision = document.getElementById('parametro_tipoemision').value; //1 = en linea, 2 fuera de liena
        let generar_cufd = document.getElementById('parametro_generarcufd').value; 
        
        
        if(tiposistema != 1 && parametro_tipoemision==1){ //si es electronico/computarizado y esta en linea
            
            if (generar_cufd == 1){ //Si esta activo la funcion de generar CUFD automaticamente
                
                var cufd_hoy = document.getElementById('cufd_hoy').value; 
                var punto_venta = document.getElementById('punto_venta').value; 
                
                if(cufd_hoy!=1){          // Sino existe un CUFD para hoy-> debe generar          
                    generarCufd(punto_venta);
                }
...
function calculardesc(){
   ...
   if(forma_id==1){//Si es efectivo
       
        $("#pago_efectivo").val(subtotal.toFixed(decimales));
        $("#pago_alternativo").val("0.00");
       
   }else{
       
       var pago_efectivo = document.getElementById('pago_efectivo').value;
       var pago_alternativo = document.getElementById('pago_alternativo').value;
       
       if(pago_alternativo <= subtotal.toFixed(decimales)){
           
           let efectivo = subtotal.toFixed(decimales) - pago_alternativo;
           $("#pago_efectivo").val(efectivo.toFixed(decimales));
            
           
       }else{           
            $("#pago_efectivo").val("0.00");
            $("#pago_alternativo").val(subtotal.toFixed(decimales));
       }
   }
...
//        }
        ...
        if (opcion==15){   //si la pulsacion proviene del codigo
                
                cliente_codigo = document.getElementById('cliente_codigo').value;     
                
                if (cliente_codigo!=''){ //Si el nit esta vacio
                    
                        var codigo = document.getElementById("cliente_codigo").value; //Generamos codigo para el cliente
                        //Si el nit es diferente de vacio                       
                        buscar_por_codigo();
...
function seleccionar(opcion) {
        ...
        if (opcion==7){
            document.getElementById('pago_alternativo').select();
        }
        
        if (opcion==8){
            document.getElementById('cliente_codigo').select();
        }
}
...
function cliente_nuevo(){
    ...
    $("#cliente_valido").val("1");
    $("#glosay").val("");
    $("#puede_facturar").val("0");
    $("#motivo_puede_facturar").val("");
    $("#cuotas").val("1");
    $("#modalidad").val("1");
    $("#dia_pago").val("1");
    $("#credito_interes").val("1");
    $("#cuota_inicial").val("0");
...
function buscar_por_codigo(){
    ...
    var base_url = document.getElementById('base_url').value;
    var codigo_cliente = document.getElementById('cliente_codigo').value;
    var parametro_factura = document.getElementById('parametro_factura').value;
    var parametro_verificarconexion = document.getElementById('parametro_verificarconexion').value;
    ...
    if (nit==''){ //Si el campo Nit esta vacio, genera NIT/Codigo automaticamente
      ...
      */  
    //Alistamos controlador para buscar al cliente
    var controlador = base_url+'venta/buscar_por_codigo';
    document.getElementById('loader_documento').style.display = 'block';
    ...
    $.ajax({url:controlador,
            ...
            success:function(respuesta){
                ...
                if (registros[0]!=null){ //Si el cliente ya esta registrado  en el sistema carga los datos
                    ...
                    $("#nit").val(registros[0]["cliente_nit"]);
                    $("#razon_social").val(registros[0]["cliente_razon"]);
                    document.getElementById('telefono').focus();
                    $("#telefono").val(registros[0]["cliente_telefono"]);
                    $("#cliente_nombre").val(registros[0]["cliente_nombre"]);
...
function cufd_vigente() {
    // Reemplazar el espacio por 'T' para que sea un formato ISO válido
    let fechaStr = document.getElementById("vigencia_cufd").value;
    let parametro_tipoemision = document.getElementById("parametro_tipoemision").value;
    const fechaComparar = new Date(fechaStr.replace(" ", "T"));
    const fechaActual = new Date();

    // Comparar fechas
    if(parametro_tipoemision==1){ //si es en linea
        
        if(fechaActual > fechaComparar){
            $("#puede_facturar").val(3); //3 cufd FUERA DE VIGENCIA
            let error = document.getElementById("motivo_puede_facturar").value + " *** CUFD FUERA DE VIGENCIA ";
            $("#motivo_puede_facturar").val(error); //2 conflicto de precios por ser cero
        }
    }
    
  
    
    //return fechaActual > fechaComparar ? 1 : 0;
}
...
/////////////////// Descripcion
                        ...
                        if (parseFloat(Number(registros[i]["producto_costo"]).toFixed(2)) >= parseFloat(Number(registros[i]["producto_precio"]).toFixed(2))) {
                            html += "<br><span class='btn btn-danger btn-xs' title='ADVERTENCIA: El precio de COSTO es MAYOR O IGUAL AL PRECIO DE VENTA...!'><fa class='fa fa-exclamation-triangle'></fa> CONFLICTO DE PRECIOS</span>";
                                ...
                                if(Number(registros[i]["detalleven_total"])==0){
                                    
                                    $("#puede_facturar").val(2); //2 conflicto de precios por ser cero
                                    let error = document.getElementById("motivo_puede_facturar").value + " *** PRECIO(S) TOTALES EN CERO ";
                                    $("#motivo_puede_facturar").val(error); //2 conflicto de precios por ser cero
                        ...
                        if( !(Number(registros[i]["producto_codigosin"])>0 && Number(registros[i]["producto_codigounidadsin"])>0 )){
                            ...
                            $("#puede_facturar").val("1"); //NO puede facturar porque existe un producto no homologado
                                    let error = document.getElementById("motivo_puede_facturar").value + " *** PRODUCTO(S) NO HOMOLOGADO(S) ";
                                    $("#motivo_puede_facturar").val(error); //2 conflicto de precios por ser cero                            
                        }
...
/////////////////// Cantidad                        
                        if(parametro_cantidadsimple==0){ //0 cantidad con botones 1 -cantidad simple        
                            ...
                            html += "                       <input type='hidden' size='1' name='productodet_id' id='productodet_"+registros[i]["detalleven_id"]+"' value='"+registros[i]["producto_id"]+"' >";
...
//                        html += "<div style='border-color: #008d4c; background: #008D4C !important; color: white' class='btn btn-success btn-xs' onclick='actualizar_losprecios("+registros[i]["detalleven_id"]+")' title='Actualizar precios'><span class='fa fa-save' aria-hidden='true'></span></div>";
                       html += "<button style='border-color: #008d4c; background: #008D4C !important; color: white' class='btn btn-success btn-xs' onclick='actualizar_losprecios("+registros[i]["detalleven_id"]+")' title='Actualizar precios'><span class='fa fa-save' aria-hidden='true'></span></button>";
                       html += "<button class='btn btn-facebook btn-success btn-xs' onclick='actualizar_portotal("+registros[i]["detalleven_id"]+")' title='Actualizar precios por el total'><span class='fa fa-calculator' aria-hidden='true'></span></button>";

                        if(parametro_botonescontrol==1){
                           html += "                            <button onclick='quitarproducto("+registros[i]["detalleven_id"]+")' class='btn btn-danger btn-xs'  title='Quitar producto de la lista'><span class='fa fa-times'></span></a></button> ";
                        }
                        html += "                        </td>";    
...
{
    var decimales = Number(document.getElementById('parametro_decimales').value);
    var forma_id = Number(document.getElementById('forma_pago').value);
    ...
    $("#venta_efectivo").val(venta_totalfinal.toFixed(decimales));
//    
//    $("#pago_alternativo").val("0.00");
//    $("#pago_efectivo").val(venta_totalfinal.toFixed(decimales));
    
     /************ INICIO PAGO COMBINADO  ***********************/  
        ...
        if(forma_id==1){//Si es efectivo
            ...
            if(Number(pago_alternativo).toFixed(decimales) <= venta_totalfinal.toFixed(decimales)){

                let efectivo = venta_totalfinal.toFixed(decimales) - pago_alternativo;
                $("#pago_efectivo").val(efectivo.toFixed(decimales));


            }else{           
                 $("#pago_efectivo").val("0.00");
                 $("#pago_alternativo").val(venta_totalfinal.toFixed(decimales));
            }
        }
...
function tabladetalle(subtotal,descuento,totalfinal)
{
    ...
    var efectivo = totalfinal - descuento;
    $("#venta_total").val(subtotal.toFixed(decimales));
    $("#venta_descuento").val(descuento.toFixed(decimales));
    $("#venta_subtotal").val(subtotal.toFixed(decimales));
    $("#venta_efectivo").val(efectivo.toFixed(decimales));
    ...
    if (Number(venta_efectivo)>0){
            $("#venta_efectivo").val(venta_efectivo.toFixed(decimales));        
    }else{        
        $("#venta_efectivo").val(efectivo.toFixed(decimales));
    }
   ...
   var check_agrupar = document.getElementById('check_agrupar').checked;
    ...
    if(Number(codigo)!=0){
       ...
       //var parametro_sininventario = document.getElementById('parametro_sininventario').value;
        ...
        if(codigobalanza==1 && codigo.length >= 12){

            var identificador =  codigo.substring(0,3);
            //alert("identificado: "+identificador);
            ...
            ///Este viene en formato CodigoProducto|PrecioTotal
            if(identificador==200){

                    resultado = procesarCodigoBarras2(codigo);
                    //alert(JSON.stringify(resultado));
                    var verificador = resultado['verificador'];
                    var codigoProducto = resultado['codigoProducto'];
                    var cantidad = Number(resultado['cantidad']);
                    var preciototal = Number(resultado['preciototal']);
        ...
        //alert("codigo: "+codigo+" cantidad: "+cantidad+" agrupado"+agrupado);
        ...
        $("#codigo").focus();
        $("#codigo").select();        
        alert("ADVERTENCIA: Código no válido...!");
...
function buscarporcodigojsx()
{
    ...
    var check_agrupar = document.getElementById('check_agrupar').checked;
    var agrupado = check_agrupar ? 1 : 0;

    document.getElementById('oculto').style.display = 'block';

    var identificador3 = codigo.substring(0, 3);
    var identificador4 = codigo.substring(0, 4);

    /*
     * BALANZA FORMATO PRECIO TOTAL
     * Ejemplo: 2000003150403
     * 2000003 = código producto
     * 1       = sin uso
     * 50403    = precio total 50.403 Bs
     */
    
/*    if (identificador4 === '2000' && codigo.length === 13) {

        var resultado = procesarCodigoBarras2(codigo);
    ...
    /*
     * BALANZAS FORMATO CÓDIGO + CANTIDAD
     */
/*    else if (identificador3 === '215') {

        var resultado = procesarCodigoBarras(codigo);

        codigo = resultado['codigoProducto'];
        cantidad = Number(resultado['cantidad']) / 1000;
    }

    else if (identificador3 === '205') {
...
{    
    ...
    if (tecla==13){
        ...
        if (Number(descuentoparcial)< Number(precio)){        
            ...
            $('#descuento'+detalleven_id).val(precio);
            tablaproductos();
...
function actualizar_losprecios(detalleven_id)
{
    ...
    if (Number(descuentoparcial) < Number(precio)){
        ...
        $.ajax({url: controlador,
                ...
                success:function(respuesta){
                    tablaproductos();
                    //calculardesc(); // Se añadio por el pago combinado
                   // tabladetalle();
...
function actualizar_portotal(detalleven_id)
{
    //var rol_modificardetalle =  document.getElementById('rol_modificardetalle').value;
    ...
    $("#calcular_detalleven_id").val(detalleven_id);
    $("#calcular_precio").val(precio);
    $("#calcular_cantidad").val(cantidad);
    
    total = Number(precio) * Number(cantidad);
    
    $("#calcular_total").val(total);
    
    $('#modalportotal').modal('show');
    $('#modalportotal').on('shown.bs.modal', function () {
        var input = $('#sumatoria_total');
        input.focus().select();
    });

}

function calcular_porcantidad()
{
    //var rol_modificardetalle =  document.getElementById('rol_modificardetalle').value;
    
    var base_url =  document.getElementById('base_url').value;
    var controlador =  base_url+"venta/actualizarprecio";
    var detalleven_id = document.getElementById("calcular_detalleven_id").value;
    var precio = document.getElementById("calcular_precio").value;
    //var descuentoparcial = document.getElementById('descuento'+detalleven_id).value; 
    var cantidad = document.getElementById("calcular_cantidad").value; 
    var total    = document.getElementById("calcular_total").value; 
    
    precio = Number(total) / Number(cantidad);    
...
function calcular_porunitario()
{
    ...
    cantidad = Number(total) / Number(precio);    
...
function registrar_calculos()
{
    ...
    var base_url =  document.getElementById('base_url').value;
    var controlador =  base_url+"venta/registrar_calculos";
    var detalleven_id = document.getElementById("calcular_detalleven_id").value;
    var precio = document.getElementById("calcular_precio").value;
    //var descuentoparcial = document.getElementById('descuento'+detalleven_id).value; 
    var cantidad = document.getElementById("calcular_cantidad").value; 
    var total    = document.getElementById("calcular_total").value; 
    var sumatoria_total    = document.getElementById("sumatoria_total").value; 

        $.ajax({url: controlador,
            type:"POST",
            data:{detalleven_id:detalleven_id, precio:precio, cantidad:cantidad, total:total, sumatoria_total:sumatoria_total},
            success:function(respuesta){
...
//        precio = precio * producto.moneda_tc;
        ...
        datos1 +="0,1,"+producto.producto_id+","+sqlString(producto.producto_codigo)+","+cantidad+","+sqlString(producto.producto_unidad)+","+costo+","+precio+","+precio+"*"+cantidad+",";
        datos1 += descuentoparcial+","+descuento+", round("+precio+"*"+cantidad+" ,2) ,"+sqlString(producto.producto_caracteristicas)+","+sqlString(preferencias)+",0,1,"+usuario_id+","+producto.existencia+",";
        datos1 += sqlString(producto.producto_nombre)+","+sqlString(producto.producto_unidad)+","+sqlString(producto.producto_marca)+",";
        datos1 += producto.categoria_id+","+sqlString(producto.producto_codigobarra)+",";        
        datos1 += producto.producto_envase+","+sqlString(producto.producto_nombreenvase)+","+producto.producto_costoenvase+","+producto.producto_precioenvase+",";
        datos1 += cantidad+",0,"+cantidad+",0,0, DATE_ADD(CURDATE(), interval "+parametro_diasvenc+" day),"+sqlString(unidadfactor)+","+preferencia_id+","+clasificador_id+","+tipo_cambio;
        //alert(datos1);
...
//    }
    ...
    if (cantidad_total <= producto.existencia){
        ...
        datos1 +="0,1,"+producto.producto_id+","+sqlString(producto.producto_codigo)+","+cantidad+","+sqlString(producto.producto_unidad)+","+costo+","+precio+","+precio+"*"+cantidad+",";
        datos1 += descuento+","+descuentoparcial+","+(precio-descuentoparcial)+"*"+cantidad+","+sqlString(producto.producto_caracteristicas)+","+sqlString(preferencias)+",0,1,"+usuario_id+","+producto.existencia+",";
        datos1 += sqlString(producto.producto_nombre)+","+sqlString(producto.producto_unidad)+","+sqlString(producto.producto_marca)+",";
        datos1 += producto.categoria_id+","+sqlString(producto.producto_codigobarra)+",";        
        datos1 += producto.producto_envase+","+sqlString(producto.producto_nombreenvase)+","+producto.producto_costoenvase+","+producto.producto_precioenvase+",";
        datos1 += cantidad+",0,"+cantidad+",0,0, DATE_ADD(CURDATE(), interval "+parametro_diasvenc+" day),"+sqlString(unidadfactor)+","+preferencia_id+","+clasificador_id+","+tipo_cambio;
        ...
        if (Number(cantidad)>0){
            ...
            if (parseFloat(cantidad_total) <= parseFloat(existencia)){
               ...
               // alert(sql);
                ...
                alert('ADVERTENCIA: La cantidad no puede ser CERO...!');
                tablaproductos();
        }
                   ...
                   if (n <= limite) x = n; 
                    ...
                    for (var i = 0; i < x ; i++){
                        ...
                        if(registros[i]["producto_foto"] != null && registros[i]["producto_foto"] !=""){
                            ...
                            //alert("con imagen: "+registros[i]["producto_foto"]);
                            ...
                            //alert("sin imagen: "+registros[i]["producto_foto"]);
...
//                                  html +=     "<button class='btn btn-success btn-xs' onclick='ingresorapido("+registros[i]['producto_id']+",1)'><b>- 1 -</b></button>";
                        ...
                        if(parametro_sininventario==1){
                            ...
                            if (parseFloat(registros[i]["existencia"])>0){
                                 ...
                                 html += "<center><small style='font-size:8px;'>SIN<br>EXISTENCIA</small><br><b style='color:red;'>"+existencia+"</b></center>";
                        ...
                        html += "<br><br>";
                        html += "<div class='modal-content'>";
                        html += "  <div class='modal-header' style='background:lightgray;'><b><fa class='fa fa-cart-arrow-down'></fa> INGRESAR ITEMS AL DETALLE</b>";
                        html += "       <button type='button' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>x</span></button>";
                        html += "  </div>";                        
                        html += "  <div class='modal-body' >";
                        
                        html += "  <!----------------------------------------------------------------->";      
/*  CODIGO DEL ANTIGUO MODAL CANTIDAD                       
...
*/                        
                        ...
                        html += "       <div class='row'>"; // style='space-white: nowrap;'
//                        html += "       <tr style='border:0px;'>"; // style='space-white: nowrap;'
//                        html += "       <td><b>Ancho:</b><input type='text' id='producto_ancho"+registros[i]["producto_id"]+"' onkeyup='calcular_cantidad("+registros[i]["producto_id"]+")'> </td>"; // style='space-white: nowrap;'
//                        html += "       <td><b>Alto:</b><input type='text'id='producto_alto"+registros[i]["producto_id"]+"' onkeyup='calcular_cantidad("+registros[i]["producto_id"]+")'> </td>"; // style='space-white: nowrap;'
//                        html += "       </tr>"; // style='space-white: nowrap;'
                        ...
                        html += "               <div class='col-md-6'>";                      
                            ...
                            html += "               <b style='font-size:14px;'>"+registros[i]["producto_nombre"]+"</b>";
                            html += "               <br><small><b>UNIDAD:</b> "+registros[i]["producto_unidad"]+" | <b>MARCA:</b> "+registros[i]["producto_marca"]+" | <b>INDUSTRIA:</b> "+registros[i]["producto_industria"]+"</small>";
                            
                        html += "               </div>";
                        
                        html += "               <div class='col-md-6'>";                      
                        html += "                   <b>  <input type='number' id='cantidad"+registros[i]["producto_id"]+"' name='cantidad"+registros[i]["producto_id"]+"'  value='1' style='font-size:20pt; width:100pt' autofocus='true' min='0' step='1' max='"+formato_numerico(registros[i]["existencia"])+"' onkeyup='registrar_cantidad(event,"+registros[i]["producto_id"]+")' ></b>";
                        html += "               </div>";                      
                        ...
                        html += "  <div class='row'>";
                            html += "  <center>";
                                ...
                                html += "     <a href='#' data-toggle='modal' data-dismiss='modal' class='btn btn-danger btn-foursquarexs'><font size='5'><span class='fa fa-times'></span></font><br><small>Cancelar</small></a>";
...
function registrarventa(cliente_id){
    ...
    let venta_efectivo = parseFloat(document.getElementById("venta_efectivo").value) || 0;
    let venta_cambio = parseFloat(document.getElementById("venta_cambio").value) || 0;
...
//    let datos_ajustesnoiva = document.getElementById('datos_ajustesnoiva').value;
    ...
    let datos_codigopais2 = document.getElementById('datos_codigopais2').value;
    let datos_incoterm = document.getElementById('datos_incoterm').value;
    let datos_incotermdetalle = document.getElementById('datos_incotermdetalle').value;
    let datos_costosgastosnacionales = document.getElementById('datos_costosgastosnacionales').value;
    let datos_totalgastosnacionalesfob = document.getElementById('datos_totalgastosnacionalesfob').value;
    let datos_costosgastosinternacionales = document.getElementById('datos_costosgastosinternacionales').value;
    let datos_totalgastosinternacionales = document.getElementById('datos_totalgastosinternacionales').value;
    let datos_codigomoneda = document.getElementById('datos_codigomoneda').value;
    let datos_tipocambio = document.getElementById('datos_tipocambio').value;
    var select = document.getElementById("datos_codigomoneda");
    var datos_monedatransaccional = select?.options?.[select.selectedIndex]?.text || ''; // select.options[select.selectedIndex].text;
    //let datos_monedatransaccional = document.getElementById('datos_monedatransaccional').value;
    let datos_descripcionpaquetes = document.getElementById('datos_descripcionpaquetes').value;
    let datos_informacionadicional = document.getElementById('datos_informacionadicional').value;
    let datos_descuentoadicional = document.getElementById('datos_descuentoadicional').value;
    let datos_lugardestino = document.getElementById('datos_lugardestino').value;
    let datos_direccioncomprador = document.getElementById('datos_direccioncomprador').value;
    let datos_puertodestino = document.getElementById('datos_puertodestino').value;
    
    let pago_alternativo = document.getElementById('pago_alternativo').value;
    let pago_efectivo = document.getElementById('pago_efectivo').value;
...
/*
    ...
    */
    ...
    var cad =   ""+forma_id+","+tipotrans_id+","+usuario_id+","+cliente_id
                ...
                +","+venta_descuentoparcial+","+venta_descuento+","+venta_total+","+venta_efectivo+","+venta_cambio+","+sqlString(venta_glosa)
                +","+venta_comision+","+venta_tipocambio+","+detalleserv_id+","+venta_tipodoc+","+tiposerv_id
                +","+entrega_id+","+sqlString(venta_numeromesa)+","+usuarioprev_id+","+pedido_id+","+orden_id+","+entregaestado_id+","+banco_id
                +","+venta_ice+","+venta_giftcard+","+sqlString(venta_detalletransaccion);
    ...
    if (tipo_transaccion==2){ //Si es al credito
        ...
        $.ajax({url: controlador,
            ...
            data:{cad:cad, tipo_transaccion:tipo_transaccion, cuotas:cuotas, cuota_inicial:cuota_inicial, 
                ...
                datos_codigopais2 : datos_codigopais2,datos_incoterm : datos_incoterm,datos_incotermdetalle : datos_incotermdetalle,
                datos_costosgastosnacionales : datos_costosgastosnacionales,datos_totalgastosnacionalesfob : datos_totalgastosnacionalesfob,
                datos_costosgastosinternacionales : datos_costosgastosinternacionales,datos_totalgastosinternacionales : datos_totalgastosinternacionales,
                datos_codigomoneda : datos_codigomoneda,datos_tipocambio : datos_tipocambio,datos_descripcionpaquetes : datos_descripcionpaquetes,
                datos_informacionadicional : datos_informacionadicional,datos_descuentoadicional : datos_descuentoadicional,
                datos_lugardestino: datos_lugardestino, datos_direccioncomprador:datos_direccioncomprador, datos_puertodestino:datos_puertodestino, datos_monedatransaccional:datos_monedatransaccional,
                pago_alternativo:pago_alternativo, pago_efectivo: pago_efectivo
            },
            success:function(respuesta){
...
function ventas_por_parametro()
{
    ...
    if (parametro!=''){
        ...
        if (Number(parametro)>0)        
            filtro = " and (v.venta_id = "+parametro+") or (cliente_codigo='"+parametro+"') ";
        else
            filtro = " and (cliente_razon like '%"+parametro+"%') or (cliente_codigo='"+parametro+"') ";
        ...
        success: function(response){
            ...
            $("#resventa").val(myString);
                ...
                for (var i=0; i< v.length; i++){    
                    ...
                    html += "                       <td style='max-width: 5cm; padding:0;' bgcolor='"+v[i]['estado_color']+"'  class='celda-posicion-relative'><font size='3'><b> "+nombre_cliente+"</b></font><sub>  ["+v[i]['cliente_id']+"]</sub>";
                    ...
                    html += "                           <span style='font-size: 10px;' class='span-superior-derecha'><b>"+v[i]['cliente_codigo']+"</b></span>";
                    html += "                       </td>";

                    //CAMPO CODIGO
//                    html += "<td style='max-width: 0.5cm; padding:0;' bgcolor='"+v[i]['estado_color']+"'><font size='3'><b> "+v[i]['cliente_codigo']+"</b></font><sub></sub>";
                    ...
                    html += "                               Pago Alternativo "+parametro_moneda_descripcion+": "+Number(v[i]['venta_pagoalternativo']).toFixed(decimales)+"<br>";
                        html += "                               Efectivo "+parametro_moneda_descripcion+": "+Number(v[i]['venta_pagoefectivo']).toFixed(decimales);
                    ...
                    if(v[i]['factura_enviada'] == 1){
                        ...
                        if(v[i]['factura_enviada'] == 2){
                            ...
                            if(v[i]['factura_enviada'] == 3){
                                        ...
                                        html += "<fa class='fa fa-chain-broken'> </fa> <small>FACT.Nº: "+v[i]['factura_numero']+" NO VALIDADA<sub>["+v[i]['factura_id']+"]</sub></small> </button>";                                                                   
                                        //Boton quitar generar facturar
                                        html += "<a class='btn btn-facebook btn-sm' data-toggle='modal' style='background: black; width:23px; height:23px; padding-left: 7px;' data-target='#modalanular_noenviada' title='Quitar asociacion de factura y emitir una nueva' onclick='cargar_modal_anular_malemitida("+v[i]["factura_id"]+","+v[i]["venta_id"]+","+v[i]["factura_numero"]+","+'"'+v[i]["factura_razonsocial"]+'"'+","+v[i]["factura_total"]+","+'"'+v[i]["factura_fecha"]+'"'+")'>";
                                        html += "<fa class='fa fa-times'> </fa> </a>";                                            
...
//                    html += "                           <a href='"+base_url+"venta/nota_venta/"+v[i]['venta_id']+"' class='btn btn-success btn-xs'><span class='fa fa-print'></span></a> ";
                    ...
                    if (v[i]['venta_tipodoc']==1){
                        ...
                        if(v[i]['factura_enviada'] == 1){
                            ...
                            html += " <a onclick='modal_enviowhatsapp("+v[i]['factura_id']+","+(Number(v[i]['cliente_celular'])>0?v[i]['cliente_celular']:'')+")' class='btn btn-success btn-xs' title='Enviar factura por whatsapp'><span class='fa fa-whatsapp'></span></a>";
                    ...
                    if (v[i]['venta_tipodoc']==1){
                            ruta      = v[i]['factura_ruta'];
                            cuf       = v[i]['factura_cuf'];
                            tamanio   = v[i]['factura_tamanio'];
                            nit_emisor   = v[i]['factura_nitemisor'];
                            num_fact   = v[i]['factura_numero'];
    ...
    document.getElementById('boton_finalizar').style.display = 'none'; //mostrar el bloque del loader
        ...
        $.ajax({url: controlador,
            ...
            data:{venta_id:venta_id, cliente_id:cliente_id, forma_pago:forma_pago, tipo_transaccion:tipo_transaccion,
                  ...
                  venta_detalletransaccion:venta_detalletransaccion, venta_glosa:venta_glosa,pago_efectivo:pago_efectivo, pago_alternativo:pago_alternativo },
            success:function(respuesta){
        ...
        success:function(respuesta){
                ...
                for(var i=0; i<p.length; i++){
                    ...
                    // alert(p[i]['usuario_id']);
                        ...
                        html += "     <br><b>FEC. ENTREGA:</b> "+formato_fecha(p[i]['orden_fechaentrega']);
                        ...
                        html += "      ";
                        html += "     ";
                        html += "     <td align='right' style='white-space: nowrap'><font size='3'><b>Total: "+Number(p[i]['orden_total']).toFixed(decimales)+"</b></font><br>";
                        html += "	 a Cuenta: "+Number(p[i]['orden_acuenta']).toFixed(decimales)+"<br>  ";
                        html += "	    Saldo: "+Number(p[i]['orden_saldo']).toFixed(decimales)+"";
                        html += "     </td>";
            ...
            success:function(resultado){
                ...
                if (registros.length>0){
                    ...
                    for (var i=0; i< registros.length; i++){
                        ...
                        if (registros[i]['detallefact_codigo']!=null || registros[i]['detallefact_codigo'] !='' || registros[i]['detallefact_codigo']!='-'){
                            codigo_producto = registros[i]['detallefact_codigo'] 
                        }else{
                            codigo_producto = "<b style='color: red'>SIN CODIGO</b>"                             
                        }
                        
                        html += codigo_producto+" "+registros[i]['detallefact_descripcion'];
                        ...
                        if(registros[i]['detallefact_caracteristicas'].length>0 && registros[i]['detallefact_caracteristicas']!='null' && registros[i]['detallefact_caracteristicas']!='-'  && registros[i]['detallefact_caracteristicas']!=' ' ) {
                    ...
                    $("#generar_monto").val(Number(registros[0]['venta_total']).toFixed(decimales));
...
function enviarfactura_porcorreo(){
    ...
    alert(venta_id+" *** "+factura_id+" *** "+elcorreo);
...
function generarCufd(punto_venta = 0) {
    ...
    $.ajax({
        ...
        success: function (datos) {
            ...
            if (lafalla && lafalla !== "" && Object.keys(lafalla).length > 0) {

                let mensaje = "No se pudo generar el C.U.F.D.\n\n";

                mensaje += registros + "\n\n";

                if (lafalla.codigo) {
                    mensaje += "Código: " + lafalla.codigo + "\n";
                }

                if (lafalla.mensaje) {
                    mensaje += "Detalle: " + lafalla.mensaje;
                } else {
                    mensaje += "Detalle: " + JSON.stringify(lafalla);
                }
            ...
            if (respuestaCufd.transaccion === true) {

                almacenar_cufd(respuestaCufd, punto_venta);

                $("#vigencia_cufd").val(formatearFecha(respuestaCufd.fechaVigencia));
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            ...
            if (jqXHR.responseJSON) {
                ...
                let datos = jqXHR.responseJSON;

                mensaje += datos.respuesta ? datos.respuesta + "\n\n" : "";

                if (datos.falla) {
                    if (datos.falla.codigo) {
                        mensaje += "Código: " + datos.falla.codigo + "\n";
                    }

                    if (datos.falla.mensaje) {
                        mensaje += "Detalle: " + datos.falla.mensaje;
                    } else {
                        mensaje += "Detalle: " + JSON.stringify(datos.falla);
                    }
                }
...
function finalizarventa_sin(){
    ...
    var venta_cambio = document.getElementById('venta_cambio').value;
    var puede_facturar = document.getElementById('puede_facturar').value;
    var todobien = false;
    var cliente_excepcion = 0;
    ...
    if (boton_presionado == 0){ //Si el boton no esta presionado
        ...
        if(Number(venta_cambio)>=0){
                ...
                if(parametro_tiposistema == 1){ //facturacion computarizada SFV
                    ...
                    finalizarventa();
                        ...
                        //verificamos el codigo de excepcion
                        if($('#codigoexcepcion').is(':checked'))
                        {    cliente_excepcion = 1; }
                        else
                        {    cliente_excepcion = 0; }    
                        ...
                        if (formasValidas.includes(forma_id)) {
                        ...
                        //Validamos si es venta facturada
                        
                        if ($('#facturado').is(':checked')){ //Si es venta facturada
                            ...
                            if(docsec_codigoc == 23 && detallebolsa == 0){
                                ...
                                location.reload();
                                        ...
                                        if(puede_facturar==0){ // puede_facturar == 0 si puede, == 1 producto sin homologar
                                        ...
                                        let error = document.getElementById("motivo_puede_facturar").value;  
                                        alert("NO PUEDE FACTURAR, MOTIVO(s): \n"+error+"\nCorrija el error y vuelva a intentar por favor..!");
                                        $("#motivo_puede_facturar").val("");
                                    }
        ...
        console.log("Finalizar venta + de una vez");
    }
...
function placa_atendida_hoy(numeroplaca) {
    let base_url = document.getElementById('base_url').value;
    let controlador = base_url + 'venta/buscar_placa_atendida';

    let resultado = false; // valor por defecto
...
function reajustar_totales() {
    
    let base_url = document.getElementById('base_url').value;
    let controlador = base_url + 'venta/reajustar_totales';
    let venta_id = document.getElementById('generar_venta_id').value; // valor de la venta
    let resultado = false; // valor por defecto
...
function buscar_placa(e){    
    ...
    if (tecla==13){
        ...
        if(!placa_atendida_hoy(numeroplaca)){
...
//                    }
            ...
            alert("NOTIFICACION: La placa: "+numeroplaca+" ya tiene registro de venta del dia..!");
        }
    }
}
...
function procesarCodigoBarras2(codigo) {

    codigo = String(codigo).trim();

    var verificador = codigo.substring(12, 13);
    var codigoProducto = codigo.substring(0, 7);
    var separador = codigo.substring(7, 8);

    // Precio: posiciones 8 a 11
    // Ejemplo: 5040 = 50.40 Bs
    var preciototal = Number(codigo.substring(8, codigo.length)) / 1000;
    ...
    if (codigo.length !== 13) {
        return {
            error: true,
            mensaje: 'Código de balanza inválido. Debe tener 13 dígitos.'
        };
    }

    if (codigo.substring(0, 4) !== '2000') {
        return {
            error: true,
            mensaje: 'El código no corresponde a etiqueta de balanza.'
        };
    }

    var base_url = document.getElementById('base_url').value;
    var controlador = base_url + 'venta/buscarproducto';
    ...
    $.ajax({
        ...
        success: function(respuesta) {
            ...
            if (Array.isArray(registro) && registro.length > 0) {
                ...
                precio = Number(registro[0]["producto_precio"]);

                if (precio > 0 && preciototal > 0) {
                    
                    cantidad = preciototal / precio;                    
    ...
    //alert(precio);
    //alert(cantidad+" ** "+precio+" ** "+preciototal);
            ...
            $.ajax({url: controlador,
                ...
                success:function(respuesta){
                    ...
                    for(var i=0; i<vg.length;i++){
                        ...
                        html += "<button class='btn btn-warning btn-xs' onclick='cargar_venta("+JSON.stringify(vg[i]["codigo_venta"])+")' title='"+vg[i]["codigo_venta"]+" "+vg[i]["nombre_venta"]+" RESP.:"+vg[i]["usuario_nombre"]+"'><fa class='fa fa-cart-arrow-down'></fa> <br>"+Number(vg[i]["totalbs"]).toFixed(2)+"<br>"+"</button>";
            ...
            $.ajax({url: controlador,
                ...
                success:function(respuesta){
                    ...
                    if (res.length>0) {  
                        ...
                        buscarcliente();  
                        //quitamos facturacion
                        document.getElementById('facturado').checked = false;
                        
                        
//                        $("#razon_social").val(res[0]["almacen_descripcion"]);
                    }
                },
                error:function(respuesta){
                    res = 0;
...
function ver_factura(){
    ...
    let base_url = document.getElementById('base_url').value;
    let controlador = base_url+'venta/ver_factura';
    ...
    $.ajax({url: controlador,
        ...
        success:function(respuesta){
            ...
            if (res.length>0) {  
                    ...
                    $("#numero_whatsapp").val(res[0]["cliente_celular"]);
                    $("#factura_idwhats").val(res[0]["factura_id"]);
                    $("#boton_whatsapp").click();
                
            }else{
                alert("No tiene una factura emitida recientemente..!");
            }
        },
        error:function(respuesta){
            res = 0;
...
function modal_enviowhatsapp(factura_id,telefono){
...
//                        document.getElementById('facturado').checked = false;     
        ...
        alert("ADVERTENCIA: No se puede realizar el envio, ingrese un numero valido y vuelva a intentar.");
    }
...
function cambiar_actividad(){
    ...
    let base_url = document.getElementById('base_url').value;
    let controlador = base_url+'venta/cambiar_actividad';
    let actividad = document.getElementById('select_actividad').value;
    
     
    var r = confirm("ADVERTENCIA: Esta seguro de cambiar la actividad. \n ¿Desea Continuar?");
                ...
                $.ajax({url: controlador,
                    ...
                    success:function(respuesta){
                        ...
                        if(res){
                            ...
                            alert("Se cambió la actividad con éxito..!");
                            location.reload();
                        }
...
function cambiar_lista_precios(){    
    let base_url = document.getElementById('base_url').value;
    let controlador = base_url+'venta/cambiar_lista_precios';
    let select = document.getElementById('select_precios').value;
    
     
    var r = confirm("ADVERTENCIA: Esta operación modificará la lista de precios por defecto de forma temporal. \n ¿Desea Continuar?");
            ...
            if(Number(select)==0){
                ...
                $.ajax({url: controlador,
                    ...
                    success:function(respuesta){
                        ...
                        if(res){
                            alert("Los precios fueron modificados...!");
                        }
...
function guardar_temporal(){
    ...
    let base_url = document.getElementById('base_url').value;
    let controlador = base_url+'venta/guardar_temporal';
    ...
    $.ajax({url: controlador,
        ...
        error:function(respuesta){
            //alert("AVISO: Verifique que tenga habilitada la tabla parametro_temporal...!");
        }
    });
...
//    alert(decimales);
    //alert(factura_id+" *** "+venta_id+" *** "+factura_numero+" *** "+factura_razon+" *** "+factura_total+" *** "+factura_fecha);
...
function anular_factura_electronica_malemitida()
{
    ...
    var base_url = document.getElementById('base_url').value;
    var controlador = base_url+'factura/quitar_factura_malemitida/'+factura_id+"/"+venta_id;
        ...
        if (r == true) {
            ...
            $.ajax({url:controlador,
                    ...
                    success:function(result){
                       ...
                       if(res){
                           cargar_factura(venta_id);
                       }else{ alert("ADVERTENCIA: Anulación Fallida..!"); }
...
function mostrar_complemento(){
    var tipo_doc_identidad = document.getElementById("tipo_doc_identidad").value;
    
    
    if(tipo_doc_identidad==1){
        document.getElementById('div_complemento').style.display = 'block';        
    }else{
        document.getElementById('div_complemento').style.display = 'none';                
    }
...
function habilitar_pagoalternativo() {

    let input = document.getElementById("pago_alternativo");
    let boton = document.getElementById("btn_pagoalternativo");
    let icono = document.getElementById("icon_pagoalternativo");
    
    var base_url = document.getElementById('base_url').value;
    var controlador = base_url+'venta/bitacora_pago/';   
    let operacion = 0;
                ...
                if (input.disabled) {
                    ...
                    $("#pago_alternativo").val(venta_totalfinal);
                    $("#pago_efectivo").val("0.00");
                    //alert("por aquiii");
                    operacion = 0;
                }
