...
function mostrar_grafica(){
    ...
    var options={
     // Build the chart
        ...
        title: {
            text: 'Venta por Rutas ('+nombre_moneda+')'
        },
        tooltip: {
    ...
    var controlador = base_url+"reportes/repventa_zonas";
            ...
            success:function(respuesta){
                ...
                if (datos != null){
                    ...
                    for(i=0;i<=totattipos-1;i++){
                        var idTP=tippos[i].zona_id;
                        var objeto= {name: tippos[i].zona_nombre, y:numeropublicaciones[idTP] };     
                    ...
                    if(tipousuario_id == 1){
                        ...
                        var optionsu={
                         // Build the chart
                            ...
                            title: {
                                text: 'Utilidades por Ruta ('+nombre_moneda+')'
                            },
                            tooltip: {
                                pointFormat: '{series.name}: <b>{point.y}</b>'
                            },
                        ...
                        for(i=0;i<=totattipos-1;i++){
                            ...
                            var idTP=tippos[i].zona_id;
                            var objeto= {name: tippos[i].zona_nombre, y:parseFloat(utilidadu) };     
...
function venta_porzona(){
    ...
    var lamoneda_id   = document.getElementById('lamoneda_id').value;
    var zona_id = $("#zona_id option:selected").val();
    var nom_cat = $("#zona_id option:selected").text();
	...
	var filtro = " date(venta_fecha) >= '"+desde+"'  and  date(venta_fecha) <='"+hasta+"' "+lazona+" ";
  //simplemente(filtro);
    $.ajax({url: controlador,
           type:"POST",
           data:{filtro:filtro},
