...
//        $this->load->model('Operaciones_model');
    ...
    function index()
    {
        ...
        if($this->acceso(53)){
            ...
            //$this->load->model('Parametro_model');
            $sql = "select *,
                    if(licencia_fechalimite>date(now()),'VIGENTE','VENCIDA') as vigencia
                    from licencia";
            $data['licencia'] = $this->Venta_model->Consultar($sql);
            $data['parametro'] = $this->Parametro_model->get_parametros();
            $this->load->model('Moneda_model');
            $data['moneda'] = $this->Moneda_model->get_moneda(2); //Obtener moneda extragera
            $data['lamoneda'] = $this->Moneda_model->getalls_monedasact_asc();
            
            $data['_view'] = 'operaciones/index';
            $this->load->view('layouts/main',$data);
    ...
    function buscarfecha()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($filtro == null){
                $result = $this->Operaciones_model->get_all_ingresos();
            }
            else{
                $result = $this->Operaciones_model->fechaingreso($filtro,$categoria);            
            }
    ...
    function add()
    {
        ...
        if($this->acceso(54)){
            ...
            if($this->form_validation->run())
            {
                ...
                $numrec = $this->Operaciones_model->numero();
                ...
                $ingreso_id = $this->Operaciones_model->add_ingreso($params);
                ...
                if($facturado=="on"){ //si la venta es facturada
                    ...
                    $this->Operaciones_model->ejecutar($sql);
                    ...
                    $factura_id = $this->Operaciones_model->ejecutar($sql);
    ...
    function edit($ingreso_id)
    {
        ...
        if($this->acceso(55)){
            ...
            $data['ingreso'] = $this->Operaciones_model->get_ingreso($ingreso_id);
            $parametro = $this->Parametro_model->get_all_parametro();
            $data['parametro'] = $parametro;
            $this->load->model('Moneda_model');
            $all_moneda = $this->Moneda_model->getalls_monedasact_asc();
            ...
            if(isset($data['ingreso']['ingreso_id']))
            {
                if(isset($_POST) && count($_POST) > 0)     
                {
                    ...
                    $this->Operaciones_model->update_ingreso($ingreso_id,$params);            
                    ...
                    $this->load->model('Banco_model');
                    $data['all_banco'] = $this->Banco_model->getall_bancosact_asc();
                    $data['page_title'] = "Modificar Operaciones";
    ...
    public function pdf($ingreso_id){
        ...
        if($this->acceso(58)){
            ...
            $data['ingresos'] = $this->Operaciones_model->get_ingresos($ingreso_id);
            $data['empresa'] = $this->Empresa_model->get_empresa(1);
            $this->load->model('Moneda_model');
            $data['lamoneda'] = $this->Moneda_model->getalls_monedasact_asc();
            $data['page_title'] = "Operaciones"; 
    ...
    public function boucher($ingreso_id){
        ...
        if($this->acceso(58)){
            $data['parametro'] =  $parametros = $this->Parametro_model->get_parametros();
            $data['ingreso'] = $this->Operaciones_model->get_ingresos($ingreso_id);
            $data['empresa'] = $this->Empresa_model->get_empresa(1);
            $this->load->model('Moneda_model');
            $data['lamoneda'] = $this->Moneda_model->getalls_monedasact_asc();
            $data['page_title'] = "Operaciones"; 
    ...
    function remove($ingreso_id)
    {
        $data['sistema'] = $this->sistema;
        if($this->acceso(56)){
        $ingreso = $this->Operaciones_model->get_ingreso($ingreso_id);

        // check if the ingreso exists before trying to delete it
        if(isset($ingreso['ingreso_id']))
        {
            $this->Operaciones_model->delete_ingreso($ingreso_id);
            redirect('ingreso/index');
        }
        else
    ...
    public function eliminar_cisession()
    {
        
        $sql = "truncate ci_session";
    ...
    public function eliminar_logs()
    {
        ...
        $ruta_logs = $this->input->post("directorio_logs");
        $archivos = glob($ruta_logs . '*'); // Obtiene todos los archivos en la carpeta logs
   
        //var_dump($ruta_logs);
        var_dump($archivos);
        foreach ($archivos as $archivo) {
            
                echo "<br>llega aqui"; // Elimina el archivo
            if (is_file($archivo)) {
                
                //echo "<br>".$archivo; // Elimina el archivo
                unlink($archivo); // Elimina el archivo
            }
        }

        echo json_encode("Archivos de logs eliminados correctamente.");
    }  
    ...
    public function lista_backups()
    {
        $ruta_backups = FCPATH . 'resources/'; // Ruta completa de la carpeta XML
        $archivos = glob($ruta_backups . '*.zip'); // Obtiene todos los archivos en la carpeta logs
...
//        }
    ...
    {
        $ruta_xml = FCPATH . 'resources/xml/'; // Ruta completa de la carpeta XML
        $archivos = glob($ruta_xml . '*'); // Obtiene todos los archivos en la carpeta
        ...
        echo json_encode("Archivos XML eliminados correctamente.");
    }
    
    
    
    public function ultimo_costo()
    {
        
        $sql = "update producto set
                producto_costo =  producto_ultimocosto";
        $this->Venta_model->ejecutar($sql);
    ...
    public function actualizar_licencia()
    {
        ...
        $licencia_fechalimite = $this->input->post("licencia_fechalimite");
        
        
        $sql = "update licencia set
                licencia_fechalimite =  '{$licencia_fechalimite}'";
        $this->Venta_model->ejecutar($sql);
    ...
    public function seleccionar_tabla()
    {
        
        $tabla_id = $this->input->post("tabla_id");
        
        
        $sql = "update tablas_sistema set
                tabla_seleccion =  if(tabla_seleccion=0,1,0)
    ...
    public function lista_tablas()
    {
        ...
        $sql = "select * from tablas_sistema";
    ...
    public function eliminar_datos()
    {
        ...
        $sql = "select * from tablas_sistema
                where tabla_seleccion = 1";        
        $tablas = $this->Venta_model->consultar($sql);
        
        if($tablas!=null){
            
            foreach($tablas as $tabla){
                
                $sql = "truncate ".$tabla["tabla_nombre"];
                $this->Venta_model->ejecutar($sql);
...
//    function compressDirectory($source, $destination) {
    ...
    function eliminar_archivo() {
        ...
        $rutaArchivo = $this->input->post("archivo");
...
//    // Uso de la función
//    $sourceDirectory = __DIR__ . '\\resources\\xml';
//    $destinationZip = __DIR__ . '\\xml_comprimido.zip';
//
//    if (compressDirectory($sourceDirectory, $destinationZip)) {
//        echo "Compresión exitosa: $destinationZip";
//    } else {
