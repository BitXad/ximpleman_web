...
//    }
    ...
    /**
     * Obtener monedas con descripción del estado.
     * Valores esperados: estado_id = 1 ACTIVO, estado_id = 2 INACTIVO.
     */
    function get_all_monedas(){
        $sql = "SELECT m.*,
                       COALESCE(e.estado_descripcion,
                           CASE WHEN m.estado_id = 1 THEN 'ACTIVO' ELSE 'INACTIVO' END
                       ) AS estado_descripcion,
                       e.estado_color
                FROM moneda m
                LEFT JOIN estado e ON e.estado_id = m.estado_id
                ORDER BY m.moneda_codigoclasificador ASC, m.moneda_descripcion ASC";
        return $this->db->query($sql)->result_array();
    }

    /**
     * Cambiar estado de una moneda específica.
     */
    function cambiar_estado_moneda($moneda_id, $estado_id){
        ...
        return $this->db->update('moneda', array('estado_id' => $estado_id));
    }

    /**
     * Cambiar estado de todas las monedas.
     */
    ...
    /**
     * Obtener descripción del estado para moneda.
     */
    function get_estado_moneda($estado_id){
        $estado_id = (int)$estado_id;
        $estado = $this->db->query(
            "SELECT estado_id, estado_descripcion, estado_color
             FROM estado
     ...
     * Total de monedas.
     */
    function contar_monedas(){
        return (int)$this->db->count_all('moneda');
    }
