...
class Encomienda extends CI_Controller
{
    ...
    public function recepcion_destino(){
        if(isset($_POST) && count($_POST)>0){
            ...
            if($e){ $this->Encomienda_model->cambiar_estado($e['encomienda_id'],5,$this->session_data['usuario_id'],$this->input->post('observacion')); redirect('encomienda/view/'.$e['encomienda_id']); }
    ...
    public function entrega($id){
        ...
        if(isset($_POST) && count($_POST)>0){
            ...
            $this->Encomienda_model->entregar($id,$params,$this->session_data['usuario_id']); redirect('encomienda/view/'.$id);
        }
    ...
    private function generar_qr_encomienda($cadena,$sufijo){
        ...
        $params=array(
            ...
            'level'=>'H',
            'size'=>5,
            'savename'=>FCPATH.'resources/images/'.$archivo
        );
