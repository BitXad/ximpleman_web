...
<link href="<?php echo $raiz;?>css/skdslider.css" rel="stylesheet" type="text/css" media="all" />
<!-- font-awesome icons -->
...
<!-- here stars scrolling icon -->
    <script type="text/javascript">
        $(document).ready(function() {
            // Evita que un error de UItoTop detenga los demas scripts de la pagina
            if (typeof $.fn.UItoTop === 'function') {
                $().UItoTop({ easingType: 'easeOutQuart' });
            } else {
                console.warn('UItoTop no esta disponible; se continua sin bloquear la pagina.');
            }
        });
    </script>
...
<script>
    // Mini Cart: no permitir que un error aqui bloquee el slider
    try {
        if (window.paypal && paypal.minicart) {
        ...
        console.warn('MiniCart no pudo inicializarse:', e);
    }
</script>
<!-- main slider-banner -->
<?php $raiz = base_url('resources/web/'); ?>
...
(function($){
    function iniciarSlider() {
        try {
            ...
            console.log('SKDSlider disponible:', typeof $.fn.skdslider);
            console.log('Cantidad de slides:', $('#demo1 > li').length);

            if (!$('#demo1').length) {
                console.error('No existe el elemento #demo1');
                return;
            }

            if (typeof $.fn.skdslider !== 'function') {
                console.error('skdslider.min.js no se cargo o produjo un error.');
                return;
            }

            // Limpieza preventiva por si hubo un intento previo de inicializacion
            if ($('#demo1').parent().hasClass('skdslider')) {
                console.warn('El slider ya estaba envuelto; no se inicializa dos veces.');
                return;
            }
            ...
            console.log('SKDSlider inicializado correctamente.');
        } catch (e) {
            console.error('ERROR al inicializar SKDSlider:', e);
        }
    }

    // NO usamos $(document).ready() para que un error de otro callback ready
    // no impida ejecutar el slider en jQuery 1.x.
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function(){
            setTimeout(iniciarSlider, 0);
        });
    } else {
        setTimeout(iniciarSlider, 0);
    }
