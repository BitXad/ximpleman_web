...
class PHPExcel_Calculation
{
    ...
    public function parseFormula($formula)
    {
        // Validación básica para verificar si realmente es una fórmula
        // Devolvemos un array vacío si no lo es
        $formula = trim($formula);
        if (empty($formula) || $formula[0] !== '=') {
            return array();
        }

        // Eliminamos el carácter '=' de la fórmula
        $formula = ltrim(substr($formula, 1));
        if (empty($formula)) {
            return array();
        }

        // Analizamos la fórmula y devolvemos el stack de tokens
        return $this->_parseFormula($formula);
    }
